...
1. sebelum packet di submit, setiap task yang masuk akan diubah menjadi `WaitingTask` dan menambahkan `done_counter` pada packet yang menampungnya, `WaitingTask` menampung informasi-informasi yang dibutuhkan, salah satunya adalah index dari packet yang `WaitingTask` itu tempati.
